...
/* device file */
static DEFINE_SEMAPHORE(dev_mutex);
...
static int rlocate_sb_mount( const char *dev_name,
...
static struct security_hook_list rlocate_security_hooks[] = {
        MY_HOOK_INIT(inode_create, rlocate_inode_create),
        MY_HOOK_INIT(inode_mkdir, rlocate_inode_mkdir),

        MY_HOOK_INIT(inode_link, rlocate_inode_link),
        MY_HOOK_INIT(inode_symlink, rlocate_inode_symlink),
        MY_HOOK_INIT(inode_mknod, rlocate_inode_mknod),
        MY_HOOK_INIT(inode_rename, rlocate_inode_rename),
        MY_HOOK_INIT(sb_mount, rlocate_sb_mount),
        MY_HOOK_INIT(sb_umount, rlocate_sb_umount),
        MY_HOOK_INIT(inode_permission, rlocate_inode_permission),
...
static int rlocate_init_procfs(void)
{
    ...
    p = proc_create("rlocate", 00600, NULL, &proc_rlocate_ops);
...
static int rlocate_dev_register(void)
{
    ...
    device_create(rlocate_class, NULL, MKDEV(Major, 0), NULL, 
                        DEVICE_NAME);
    // devfs
...
{
    device_destroy(rlocate_class, MKDEV(Major, 0));
...
static int rlocate_sb_mount( const char *dev_name, const struct path *path, const char *type, unsigned long flags, void *data)
{
	RELOAD = '1';
    return 0;
}
...
//TODO:    
    ...
    /*     return 0; */
    /* } */
    /* dentry = list_entry(d_list.next, struct dentry, d_u.d_alias ); */
...
static int fuck_security_add_hooks(void)
{
    int idx;
    ...
    for (idx = 0; idx < ARRAY_SIZE(rlocate_security_hooks); idx++)
        rlocate_security_hooks[idx].head = ((void *) hooks)
            + ((unsigned long) rlocate_security_hooks[idx].head)
            - ((unsigned long) &probe_dummy_security_hook_heads);
    
    for (idx = 0; idx < ARRAY_SIZE(rlocate_security_hooks); idx++)
		add_hook(&rlocate_security_hooks[idx]);
    return 0;
}
/*
 * init_rlocate()
...
static int __init init_rlocate(void)
{
    ...
    ret = fuck_security_add_hooks();
    printk(KERN_INFO "rlocate fuck LSM : %d\n", ret);
...
no_exclude_dir_arg:
    kfree(path_buffer); 
no_path_buffer:
    //security_delete_hooks(rlocate_security_hooks, ARRAY_SIZE(rlocate_security_hooks));
    rlocate_dev_unregister();
out:
    return ret;
}
